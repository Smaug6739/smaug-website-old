...
export function auth(req: IObject, res: IObject): void {
    Members.auth(req.body.username, req.body.password)
        .then(result => {
            ...
            res.cookie('user_token', `${token}`, { maxAge: 3600000, httpOnly: true, secure: process.env.NODE_ENV == 'production' ? true : false, sameSite: process.env.NODE_ENV == 'production' ? 'None' : 'Lax' })
            res.cookie('user_id', `${result.id}`, { maxAge: 3600000, httpOnly: true, secure: process.env.NODE_ENV == 'production' ? true : false, sameSite: process.env.NODE_ENV == 'production' ? 'None' : 'Lax' })
            res.cookie('user_auth', `true`, { maxAge: 3600000, httpOnly: false, secure: process.env.NODE_ENV == 'production' ? true : false, sameSite: process.env.NODE_ENV == 'production' ? 'None' : 'Lax' })
